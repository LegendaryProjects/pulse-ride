...
const LocationDropdown = ({ placeholder, value, onChange, disabled }) => {
  ...
  const filteredLocations = NITK_LOCATIONS.filter((loc) =>
  ...
  return (
    <div className="relative w-full md:flex-1" ref={dropdownRef}>
      <div 
        className={`border border-[#282828] px-4 py-3 rounded transition-colors flex justify-between items-center ${
          disabled 
            ? "bg-[#121212] opacity-50 cursor-not-allowed" 
            : "bg-[#121212] cursor-pointer hover:bg-[#282828]"
        }`}
        onClick={() => !disabled && setIsOpen(!isOpen)}
      >
        <span className={value ? "text-white" : "text-[#B3B3B3]"}>
          {value || placeholder}
        </span>
        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-[#B3B3B3]" viewBox="0 0 20 20" fill="currentColor">
          <path fillRule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clipRule="evenodd" />
        </svg>
      </div>

      {isOpen && !disabled && (
        <div className="absolute z-10 w-full mt-2 bg-[#181818] border border-[#282828] rounded shadow-lg">
          <div className="p-2 border-b border-[#282828]">
            <input
              type="text"
              className="w-full bg-[#121212] text-white px-3 py-2 rounded focus:outline-none focus:ring-1 focus:ring-[#1ED760]"
          ...
          <ul className="max-h-60 overflow-y-auto [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-track]:bg-[#121212] [&::-webkit-scrollbar-thumb]:bg-[#282828] [&::-webkit-scrollbar-thumb]:rounded-full hover:[&::-webkit-scrollbar-thumb]:bg-[#3E3E3E] transition-colors">
            {filteredLocations.length > 0 ? (
              filteredLocations.map((loc) => (
                <li
                  key={loc}
                  className="px-4 py-2 text-[#B3B3B3] hover:bg-[#282828] hover:text-white cursor-pointer transition-colors"
                  onClick={() => {
                    onChange(loc);
                    setIsOpen(false);
                    setSearchTerm("");
              ...
              <li className="px-4 py-2 text-[#B3B3B3] italic cursor-default">No locations found</li>
            )}
          </ul>
        </div>
      )}
...
const TripSelector = ({ pickup, setPickup, dropoff, setDropoff, isConfirmed, onConfirm }) => {
  // Button is disabled if missing points OR if already confirmed
  const isButtonDisabled = !pickup || !dropoff || isConfirmed;

  return (
    <div className="flex flex-col md:flex-row gap-4 w-full p-4 bg-[#181818] rounded-lg mt-6 shadow-md">
      <LocationDropdown 
      ...
      <LocationDropdown 
        placeholder="Choose DropOff Point" 
        value={dropoff} 
        onChange={setDropoff}
        disabled={isConfirmed} 
      />
